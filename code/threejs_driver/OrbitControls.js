/**
  update: теперь этот файл не используется, берется файл из three.js-part
  todo: понять что еще мы тут меняли и портировать туда (возможно патчем в SceneControl)

 правка заключается в 1 строчке
 	function handleKeyDown( event ) {
 	...
		if (event.target.tagName != "BODY") return; // pv
	...
